...
def train_model(data_dir, num_classes, model_name, fine_tune, epochs, learning_rate, batch_size, train_split, valid_split, use_weighted_loss, l2_lambda, patience, model_id=None, run_id=None):
    ...
    if model_name in ['ResNet50', 'DenseNet121']:
        batch_size = min(batch_size, 4)  # Reduzi para 4 para modelos maiores
    ...
    # Ajuste no tamanho do lote já foi feito anteriormente
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True, worker_init_fn=seed_worker, generator=g, pin_memory=True)
    valid_loader = DataLoader(valid_dataset, batch_size=batch_size, shuffle=False, worker_init_fn=seed_worker, generator=g, pin_memory=True)
    test_loader = DataLoader(test_dataset, batch_size=batch_size, shuffle=False, worker_init_fn=seed_worker, generator=g, pin_memory=True)
...
def main():
    ...
    **Produzido pelo:**
    ...
    with st.form(key='training_form'):
        ...
        runs_per_model = st.number_input("Número de Execuções por Modelo:", min_value=1, max_value=3, value=1, step=1, key="runs_per_model")  # Limitar para 3 para testes
                    ...
                    for i, model_name in enumerate(model_list):
                        for run in range(1, runs_per_model + 1):
                            ...
                            try:
                                ...
                                st.error(f"Erro no treinamento do Modelo {model_name}, Execução {run}: {e}")
                                logging.error(f"Erro no treinamento do Modelo {model_name}, Execução {run}: {e}")
                                continue  # Continua para o próximo modelo/executação
